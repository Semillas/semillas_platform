...
const fetchFeed = (searchFilters, nextPageUrl)=> ({
  searchFilters,
  [CALL_SEMILLAS_API]: {
    types: [ FEED_REQUEST, FEED_SUCCESS, FEED_FAILURE ],
    endpoint: nextPageUrl + searchFilters,
    schema: Schemas.FEED
  }
})
...
export const loadFeed = (searchFilters, nextPage, requiredFields = []) => (dispatch, getState) => {
  const {
    nextPageUrl = `service/feed`,
    pageCount = 0
  } = getState().pagination.feedServices[searchFilters] || {}

  if (pageCount > 0 && !nextPage) {
    return null
  }

  return dispatch(fetchFeed(searchFilters, nextPageUrl))
}
